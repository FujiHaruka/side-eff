class SideEff {
  ...
  affectThen (fn) {
    ...
    if (s._isImplementedAffect()) {
      ...
      const promise = s.affectAsync(s.payload).then(() => sideEffect)
      // Bad practice
      promise.affectThen = sideEffect.affectThen.bind(sideEffect)
      promise.unwrap = sideEffect.unwrap.bind(sideEffect)
      return promise
    }
  }
